...
static struct proc_stat_info {
    int  code_macro;
    char *code_str;
    char *code_dsp;
} app_state [] = {
    { TASK_RUNNING         ,  "R", "Running or runnable (on run queue)" },
    { TASK_SLEEPING        ,  "S", "Interruptible sleep (waiting for an event to complete)" },
    //{ TASK_INTERRUPTIBLE   ,  "R", "Running or runnable (on run queue)" },
    { TASK_UNINTERRUPTIBLE ,  "D", "Uninterruptible sleep (usually IO)" },
    { TASK_STOPPED         ,  "T", "Stopped, either by a job control signal or because it is being traced" },
    { TASK_ZOMBLE          ,  "Z", "Defunct (\"zombie\") process, terminated but not" },
    {   -1       , NULL,        NULL}
};
