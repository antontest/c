 ...
 */
int inet_socket_startup(int domain, int type, struct sockaddr_in *addr, \
        const char *ip, u_short port, int is_ser)
{
    ...
    /* bind socket */
    if (is_ser) socket_bind(fd, (struct sockaddr *)addr);
 ...
 * @return socket fd, if succ; exit, if fail
 */
int local_socket_startup(int domain, int type, const char *path, int is_ser)
{
    int fd = -1;
