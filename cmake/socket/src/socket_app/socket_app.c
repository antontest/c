        ...
        const char *cast_info)
{
    ...
    if (cast_times < 1 || cast_info == NULL) return -1;

    /**
     * 1. socket create and init
     * 2. set socket broadcast
     */
    fd = socket_create(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    ...
    while (cast_times-- > 0)
    {
        if (socket_addr_sendto(fd, (void *)cast_info, 
                    strlen(cast_info), &addr) > 0)
            printf("broadcast send to %s: %s\n", \
                cast_ip, cast_info);
        sleep(1);
    }
        ...
        char *cast_info, int size)
{
    ...
    while (cast_times-- > 0)
    {
        if (socket_addr_recvfrom(fd, p, \
                size, &addr) > 0)
            printf("broadcast recv from %s: %s\n", \
                inet_ntoa(addr.sin_addr), p);
    }
