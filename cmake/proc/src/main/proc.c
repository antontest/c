...
int main(int agrc, char *agrv[])
{
            ...
            if (rt > 0) {
                ...
                for (str = buff; ; str = NULL) {
                    ...
                    if (get_cmdline(pid, buf, sizeof(buf)))
                        printf("\033[1;36mcmdline of pid %d is: \033[1;35m%s\n\033[0m", pid, buf);
                    else 
                        printf("\033[1;31mget cmdline of pid %d failed \n\033[0m", pid);
    ...
    if (pid_by_name_flag) {
        ...
        if (rt > 0)
            printf("\033[1;36mpid of %s is: \033[1;35m%s\n\033[0m", name, buf);
        else 
            printf("\033[1;31mget pid of %s failed \n\033[0m", name);
        rt = 0;
    }

    if (name_by_pid_flag) {
        if (!get_name_by_pid(pid, buf, sizeof(buf)))
            printf("\033[1;36mname of pid %d is: \033[1;35m%s\n\033[0m", pid, buf);
        else 
            printf("\033[1;31mget name of pid %d failed \n\033[0m", pid);
    }
    ...
    if (par_flag) {
        ...
        if (ppid > 0) {
            if (!get_name_by_pid(ppid, buf, sizeof(buf)))
                printf("\033[1;36mparent name of pid %d is: \033[1;35m%s\n\033[0m", pid, buf);
            else 
                printf("\033[1;31mget parent name of pid %d failed \n\033[0m", pid);
        }
    }
    ...
    if (unique_flag) {
        ...
        if (rt > 0)
            printf("\033[1;36mwhether %s is unique? : \033[1;35mno\n\033[0m", name);
        else if (!rt)
            printf("\033[1;36mwhether %s is unique? : \033[1;35myes\n\033[0m", name);
        else 
            printf("\033[1;36mcheck whether %s is unique failed\n\033[0m", name);
    }

    if (exec_path_flag) {
        if (!get_exe_path_by_pid(pid, buf, sizeof(buf)))
            printf("\033[1;36mpath of pid %d is: \033[1;35m%s\n\033[0m", pid, buf);
        else 
            printf("\033[1;31mget path of pid %d failed \n\033[0m", pid);
    }
