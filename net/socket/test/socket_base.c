...
int main(int argc, char *argv[])
{
    ...
    if ((fd = startup_local_client(SOCK_DGRAM, &addr, ipc_path)) == -1)
        return -1;

    socket_connect(fd, &addr);
    printf("connect succ\n");
    socket_send(fd, "hi, ser!", sizeof("hi, ser!"));
						...
						const char *ip, u_short port)
{
	...
	/* create socket */
    return socket_create(AF_INET, type);
}
...
int startup_local_client(int type, struct sockaddr_un *addr, const char *path)
{
    ...
    /* create socket */
    return socket_create(AF_UNIX, type);  
}
